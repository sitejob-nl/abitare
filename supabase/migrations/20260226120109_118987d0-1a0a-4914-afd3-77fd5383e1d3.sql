...
DROP FUNCTION IF EXISTS public.get_products_for_price_group(uuid,text,uuid,uuid,boolean,text,text,integer,integer,numeric,numeric);
...
RETURNS TABLE(
  id uuid, article_code varchar, name text, sku varchar, description text,
  base_price numeric, cost_price numeric, book_price numeric,
  width_mm integer, height_mm integer, depth_mm integer,
  is_active boolean, pricing_unit pricing_unit,
  kitchen_group varchar, type_code varchar, type_name_nl varchar,
  supplier_id uuid, supplier_name text, supplier_code text,
  category_id uuid, category_name text, category_code text,
  price_group_price numeric, total_count bigint
)
...
BEGIN
  ...
  RETURN QUERY
  SELECT
    p.id, p.article_code, p.name, p.sku, p.description,
    p.base_price, p.cost_price, p.book_price,
    p.width_mm, p.height_mm, p.depth_mm,
    p.is_active, p.pricing_unit,
    p.kitchen_group, p.type_code, p.type_name_nl,
    p.supplier_id,
